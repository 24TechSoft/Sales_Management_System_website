					...
					<div class="col-md-9 col-sm-8 col-xs-12">
						<div class="main-content-shop">
							<div class="product-grid-view">
								<h2 class="title18 title-widget font-bold text-center">Order history</h2>
							<?php $counts=0; foreach($CusOrders as $roworder){ ?>
								<div class="row">
									<div class="col-md-12 col-sm-12 col-xs-12 orderhead dfcolor">
									<div class="col-md-4 col-sm-6 col-xs-6">
											<label>Order Date:</label> <?php echo $roworder["OrderDate"];?></br>
											<label>Order No:</label> <?php echo $roworder["OrderCode"];?>
										</div>
										<div class="col-md-4 col-sm-6 col-xs-6">
											<label>Current Status:</label> <?php if($roworder["Status"]==0){?> Order Pending <?php } else if($roworder["Status"]==1) {?> Order Confirmed <?php } else if($roworder["Status"]==2) {?> Order Delivered <?php } else if($roworder["Status"]==3) {?>Order Cancelled <?php } else if($roworder["Status"]==4) {?>Order Returned <?php } ?></br>
											<label>Amount:</label> &#8377; <?php echo $roworder["GrandTotal"];?>
											
											 <input type="hidden" class="delstatus" id="stat<?php echo $counts;?>" value="<?php echo $roworder["Status"];?>"/>
												 <div id="del" style="display:none;">
													<input type="checkbox" value="2" id="statorder" onchange="updateOrderStatus(<?php echo $roworder["ID"];?>,this.value);"/><label>Delivered</label>
												 </div>
										</div>
										<div class="col-md-4 col-sm-6 col-xs-6">
											<label>No Of Items:</label> <?php echo sizeOf($roworder["Items"]);?></br>
											<a class="brn-cll" href="<?php echo base_url();?>Shopping/tracking/<?php echo $roworder["ID"]; ?>">Tracking Details</a>
												 
										</div>
										<div class="clearfix"></div>
										<div class="col-md-4 col-sm-6 col-xs-12"></div>
										<div class="col-md-4 col-sm-6 col-xs-12">
										<a class="brn-cll" href="<?php echo base_url();?>Cart_Controller/orderdetail/<?php echo $roworder["OrderCode"];?>/<?php echo $roworder["ID"]; ?>">Order Details</a>
										</div>
									...
									<div class="table-responsive">
										<table class="table ord">
											...
											<tbody>
											<?php $count=0; foreach($roworder["Items"] as $item){ $count++;?>
												<tr>
