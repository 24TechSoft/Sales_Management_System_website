   ...
   Class OrderMaster_Controller extends  CI_Controller{
	...
	public function index(){
		...
		if(($ID!="")||($ID!=0)||($ID!=null)){
			...
			$whereOrderID="a.OrderID=$ID and b.AssignedWarehouse=$Warehouse";
	...
	public function UpdateOrder(){	
		...
		if($Status==1){ /*when order is confirmed bill is being generated and items are added to out stock*/
			...
			$wherebill="a.OrderID = $ID order by ID desc limit 1";

			$Bill=$this->Order_Model->ShowBill($wherebill);
			...
			$wherebillitem="OrderID=$ID";

			$Items=$this->Order_Model->SelectItem($wherebillitem);
			...
			foreach($Items as $rowitems){
				...
				$databillitem=array(
					...
					'PackageQuantity'=>$rowitems["PackageQuantity"],

					'PackagePrice'=>$rowitems["PackagePrice"],

					'TotalPrice '=>$rowitems["TotalPrice"],

					'PackageID '=>$rowitems["PackageID"],

					'PackageName'=>$rowitems["PackageName"],

					'TotalQuantity'=>$rowitems["TotalQuantity"],

					

				);

			//	$this->Order_Model->AddBillItem($databillitem);
				...
				$dataoutstock = array(
						 ...
						 'Quantity'=>$rowitems["TotalQuantity"],

						 'Remarks'=>'sold',

						 );

				$outItemID=	$rowitems["ItemID"];	 

				$CurrentQuantity=$rowitems["PackageQuantity"]*$rowitems["TotalQuantity"];

				$datac="Quantity=Quantity-$CurrentQuantity";

				$whereOutstock="WarehouseID=$AssignedWarehouse and ItemID=$outItemID";

				$availability=$this->InStock_Model->itemInstock($whereOutstock);

				$quantity="";

				foreach($availability as $rowq){
				...
				'Detail'=>"Out stock from warehouse : ".$AssignedWarehouse,
			...
			$this->Order_Model->addNotification($data_notifica);

				/*notification*/

				$this->InStock_Model->updateCurrent_Stock($datac,$whereOutstock);
	...
	public function requestItem(){
		...
		$RWarehouseID=$this->uri->segment(4);
		...
		$data=array(
			...
			'FWarehouseID'=>$WarehouseID,

			'TWarehouseID'=>$RWarehouseID,
		...
		$this->Order_Model->requestItem($data);
		...
		$data_noti=array(
			...
			'Detail'=>"Item sent from".$WarehouseID."to ".$RWarehouseID,
